...
hifi_prefix=framework/services/prebuild/nuttx_audio_$PROJECT
if [ -f "$hifi_prefix.bin" ]; then
    cp "$hifi_prefix.bin" "$output/nuttx_hifi.bin"
fi

if [ -f "$hifi_prefix.elf" ]; then
    cp "$hifi_prefix.elf" "$output/nuttx_hifi.elf"
fi

if [ -f "$hifi_prefix.map" ]; then
    cp "$hifi_prefix.map" "$output/nuttx_hifi.map"
fi
