#include <time.h>
#include "../include/circle_screen.h"
#include "../include/font_manager.h"

static lv_obj_t *watchScr = NULL;

lv_obj_t * get_watch_scr(void)
{
    if (watchScr == NULL) {
        watchScr = lv_obj_create(lv_scr_act());
        if (watchScr == NULL) {
            LV_LOG_ERROR("Failed to create watch screen");
            return NULL;
        }
